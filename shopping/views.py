...
from recommend_shopping import recommend_by_user_id, recommend_by_item_id
from .forms import *
...
def movie(request, movie_id):
    ...
    movie_rate = Rate.objects.filter(movie=movie).all().aggregate(Avg('mark'))#商品评分
    if movie_rate:
...
def hot_movie(request):
    ...
    movies = Movie.objects.annotate(user_collector=Count('collect')).order_by('-user_collector')[:10]#查出所有商品，按收藏人数排序
...
def director_movie(request, director_name):
    ...
    movies = movies_paginator(movies, page_number)
    return render(request, "items.html", {"movies": movies, "title": "{}的商品".format(director_name)})
...
def make_comment(request, movie_id):
    ...
    return redirect(reverse("shopping", args=(movie_id,)))
