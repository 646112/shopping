...
def recommend_by_user_id(user_id):
    ...
    for user in users:
        ...
        if rates:
            for i in rates:
                rate.setdefault(str(i.movie.id), i.mark)#填充商品数据
...
def recommend_by_item_id(user_id, k=15):
    ...
    # 选用户最喜欢的标签中的商品，用户没看过的30个，对这30个商品，计算距离最近
    un_watched = Movie.objects.filter(~Q(rate__user_id=user_id), tags__in=user_prefer).order_by('?')[:30]  # 看过的商品
    ...
    for un_watched_movie in un_watched:
        for watched_movie in watched:
            if un_watched_movie not in names:
                ...
                distances.append((similarity(un_watched_movie.id, watched_movie[0]) * watched_movie[1], un_watched_movie))#加入相似的商品
